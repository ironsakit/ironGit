...
unsigned int *create_16_words(Sha_struct *s){
  ...
  for(int i = 0; i < DIM_WORDS; i++){  // Big Endian
...
void exchange(unsigned int *a, unsigned int *b, unsigned int *c, unsigned int *d, unsigned int *e, unsigned int *TMP){
  *e = *d;
  *d = *c;
...
void blender(Sha_struct *s, unsigned int *words_80){
...
char * SHA_1(char *string, unsigned long long int total_len){
  Sha_struct myBuffer;
